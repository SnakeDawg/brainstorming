...
- **Standalone skills** — invoked directly, not owned by any agent. They
  ...
  Standalone skills MUST ship their own `rubric.md`.
- **Shared skills** — utility skills that any agent may pull into its
  ...
  input/output pairs. Calling agents SHOULD tag rules that exercise
...
their rubric from the owning agent. See the
...
| [`onboard`](./onboard/SKILL.md) | Interactive wizard to scaffold new agents/skills with correct frontmatter and rubric templates | [`rubric.md`](./onboard/rubric.md) |
...
| [`execute`](./shared/execute/SKILL.md) | Canonical spec for match types, diff application, and ~~~test~~~ block parsing | any | (spec is self-documenting) |
...
- `scope: standalone` → lives at `skills/<name>/`, ships `rubric.md`.
- `scope: shared` → lives at `skills/shared/<name>/`, no rubric.
- `scope: dedicated` → lives at `agents/<owner>/skills/<name>/`
  (NOT here); inherits rubric from owning agent.
...
The rules are identical to agent bootstrap: `rubric.md` is
human-authored (evaluation rules + weights), and the improver refuses
to run the loop until at least one valid `~~~test~~~` rule exists and
weights sum to 1.0. See
