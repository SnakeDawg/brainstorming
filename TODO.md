...
- [ ] **Regression rule is wrong under `partial_credit: true`.**
...
- [ ] **Scorer self-test only covers `contains` and `exact`.**
...
- [ ] **Self-test fixture is prose inside SKILL.md.**
...
- [ ] **Match-types table duplicated in 3 places.**
...
- [ ] **Bootstrap and score disagree on weight-sum handling.**
...
- [ ] **Worked example is stale relative to Issues 1–9.**
...
- [ ] **`trigger: production_count` in greeter is aspirational.**
...
- [ ] **`saturated_iterations: 6` rationale missing.**
...
- [ ] **Add RESEARCH.md for external references.**
...
- [ ] **Decouple rubric-version check from HISTORY.md.**
...
- [ ] **`rebaseline` under-described in README "Test locally".**
...
- [ ] **Onboard wizard callout points at vaporware.**
  `README.md` Quickstart. Either delete or mark clearly as "proposed,
  not implemented".

- [ ] **Bootstrap should write default `improvement_policy` block.**
...
- [ ] **Run-id hex seed is deterministic.**
...
- [ ] **References duplicated between README.md and SELF-IMPROVEMENT.md.**
  Pick one home, link from the other.

- [ ] **`reasoning_chain.rejected_approaches` needs prompt pressure.**
